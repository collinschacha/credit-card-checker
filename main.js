...
// checks if the cards are valid and prints true or not
...
// check for invalid  numbers in an array
...
// id companies issuing invalid cards
const findInvalidCompanies = (arr) => {
  const invalidCompanies = [];
  const invalidCards = findInvalidCards(arr);
  for (let j = 0; j < invalidCards.length; j++){
    const firstDigit = invalidCards[j][0];
    if (firstDigit === 3) {
...
console.log(findInvalidCompanies(batch));